...
- 固定シンボル表:
    ...
    'A'..'Z' = 配置記号（床扱い。『配置』節で npc= に解決）
- 『配置』節の "- N: npc=elder" で配置記号→NPC id を対応づける
"""
...
def lint(doc: MapDoc) -> list[LintIssue]:
    ...
    if starts == 0:
        issues.append(LintIssue("error", "プレイヤー初期位置 '@' がありません"))
    elif starts > 1:
